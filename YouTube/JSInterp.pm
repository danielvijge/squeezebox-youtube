...
sub interpret_expression {
	...
	while ($expr =~ /(\(|\))/g) {
		...
		if ($parens_count == 0) {
			...
			$expr = encode_json($sub_result) . $remaining_expr;
	...
	my $func = $obj->{$member};
	$self->progress($depth, ">>", "$member.$variable(" . join(",", map { encode_json($_) } @{$argvals}) . ")");
	my $ret = &$func(@{$argvals});
	$self->progress($depth, "<<", encode_json($ret));
	return $ret;
    }
...
sub call_function {
    ...
    $self->progress($depth, ">>", "$funcname(" . join(",", map { encode_json($_) } @args) . ")");
